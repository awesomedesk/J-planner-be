...
CREATE TABLE todos (
    ...
    `sort_order`        INT             NOT NULL DEFAULT 0      COMMENT '박스 표시 순서. 새 Todo = 최댓값 + 1 (맨 아래, D-029)',
    `created_at`        DATETIME        NOT NULL DEFAULT NOW()  COMMENT '생성일시',
    `updated_at`        DATETIME        NOT NULL DEFAULT NOW()  COMMENT '수정일시',
    `deleted`           VARCHAR(1)      NOT NULL DEFAULT 'N'    COMMENT '삭제여부'
                                        CHECK (`deleted` IN ('N', 'Y')),
...
CREATE TABLE user_settings (
    ...
    `start_view`            VARCHAR(10) NOT NULL DEFAULT 'MONTH' COMMENT '처음 화면 (LAST = 브라우저 localStorage에 저장, D-029)'
...
ALTER TABLE calendars
    ...
    MODIFY COLUMN `color` VARCHAR(8) NULL COMMENT '일정 색 (NULL = 테마의 Theme2 색, D-030)',
